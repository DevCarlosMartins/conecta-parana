...
export class CitiesService {
  ...
  async remove(id: number) {
    ...
    const [usersCount, eventsCount, newsCount, localsCount] =
    ...
    if (usersCount > 0 || eventsCount > 0 || newsCount > 0 || localsCount > 0) {
      throw new ConflictException(
        'Não é possível deletar a cidade, pois existem dados vinculados a ela',
      );
    }
